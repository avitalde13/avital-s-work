...
public class Board {
    ...
    private boolean onStar(Word w) {
        ...
        // 1) Check if word coordinates are legal.
        if (col < 0 || row < 0 || col > 14 || row > 14)
            return false;

        // 2) Check if word is legal (contains no null value's)
        for (int i = 0; i < length; i++)
            if (w.tiles[i] == null)
                return false;

        // 3) Check if word on the star bonus location
        // Vertical word
        if (vertical) {
    ...
    private boolean notOnStar(Word w) {
        ...
        for (int i = 0; i < length; i++) {
            if (w.tiles[i] == null)
                return touchingTile(w);
        }
        return nearTile(w);
    }

    // Check if word touch another tile
    private boolean touchingTile(Word w) {
        int length = w.tiles.length;
        int row = w.row;
        int col = w.col;
        boolean vertical = w.vertical;
